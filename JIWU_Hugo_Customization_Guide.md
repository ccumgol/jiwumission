...
## 17. 대용량 미디어 포맷 WebP 이미지 최적화 및 본문 링크 교체

웹사이트의 리소스 로딩 속도를 향상시키고 GitHub Actions 빌드 및 GitHub Pages 호스팅 용량을 최소화하기 위해 기존의 고용량 `.png` 이미지 리소스를 모두 `.webp` 포맷으로 변환하고 마크다운 본문의 참조 경로를 일괄 업데이트했습니다.

### 1) 이미지 최적화 (WebP) 적용 배경
* **용량 최적화**: WebP는 PNG 대비 동일 화질 기준 약 26% 이상 용량이 작으며, 사이트 전체 이미지 파일 용량을 대폭 줄여 초기 접속 시 브라우저 렌더링 성능을 개선합니다.
* **빌드 안정성 확보**: Go 언어 기반의 Hugo 이미지 리사이징 파이프라인(`.Resize`) 구동 시, 일부 파일 확장자(`.png`)와 실제 인코딩 방식(실제로는 WebP 등)이 불일치하면 발생하는 `image: unknown format` 예외를 근본적으로 차단하기 위해 실질 포맷과 확장자명을 일치시켰습니다.

### 2) 적용 및 처리 절차
1. **포맷 변환 및 삭제**: 기존의 수많은 `.png` 정적 이미지들을 WebP 포맷으로 일괄 변환(품질 약 82% 무손실급 손실 압축)하여 추가하고, 원본 `.png` 파일들은 저장소 관리에서 삭제 처리했습니다.
...
## 19. 다국적 타임존 불일치 해결 및 IT 뉴스 404 링크 에러 원천 방지

해외 거주자(미국 동부 시간 등)가 접속할 때 한국 자정과 시간대 불일치로 성경 말씀(QT) 팝업 모달이 13시간 일찍 초기화되어 오작동하던 현상 및 오늘의 IT 뉴스 클릭 시 404 에러가 나던 현상을 해결하기 위해 날짜 판정 로직을 개선했습니다.

### 1) 오늘의 IT 뉴스 최신 글 템플릿 정적 바인딩 (404 방지)
* **수정 파일**: [home.html](file:///Users/gihyunpark/Desktop/jiwumission/layouts/home.html)
* **내용**:
  * 기존 자바스크립트로 계산하던 오늘의 IT 뉴스 이동 링크(`/blog/daily-it-news/YYYY-MM-DD/`) 방식을 폐지했습니다.
  * 대신, Hugo 템플릿 빌드 단계에서 카테고리가 `"매일의 IT뉴스"`인 글들 중 최신 글을 정적으로 쿼리하여 HTML 자체에 주입하도록 변경했습니다.
  * 이를 통해 아직 파일이 생성되지 않은 날짜로 인한 404 에러가 발생하지 않으며, 항상 게시된 최신 뉴스를 원클릭으로 읽을 수 있게 보장합니다.

### 2) 사용자 브라우저 로컬 날짜 기준의 QT 모달 초기화
* **수정 파일**: [home.html](file:///Users/gihyunpark/Desktop/jiwumission/layouts/home.html)
* **내용**:
  * 한국 표준시(KST)로 날짜를 강제 변환하던 `getKSTDateString()` 함수 대신 브라우저 로컬 시간을 기준으로 자정이 지나면 날짜가 변경되는 `getLocalDateString()` 함수를 도입했습니다.
  * 이로 인해 각 접속자의 현지 시간 자정을 기준으로 새로운 QT가 갱신되어 제공되며, '오늘 하루 보지 않기'의 `localStorage` 판정도 현지 기준 24시간 동안 오작동 없이 정확하게 유지됩니다.
