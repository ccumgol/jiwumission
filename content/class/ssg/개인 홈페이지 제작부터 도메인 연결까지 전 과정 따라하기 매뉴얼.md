---
title: "개인 홈페이지 제작부터 도메인 연결까지: 전 과정 따라하기 매뉴얼"
date: "2026-07-13T20:04:00.000Z"
author: "Gihyun Park"
lastmod: "2026-07-14"
summary: "이 문서는 Hugo와 GitHub, Cloudflare Pages를 이용해 개인 홈페이지를 만들고, 커스텀 도메인까지 연결하는 전체 과정을 단계별로 상세히 안내합니다."
description: "이 문서는 Hugo와 GitHub, Cloudflare Pages를 이용해 개인 홈페이지를 만들고, 커스텀 도메인까지 연결하는 전체 과정을 단계별로 상세히 안내합니다."
tags: ["잎새 39호"]
image: "images/개인-홈페이지-제작부터-도메인-연결까지-전-과정-따라하기-매뉴얼.webp"
comments: false
notion_url: "https://app.notion.com/p/39c091c284f68043a6ccd29919234c1f"
notion_id: "39c091c2-84f6-8043-a6cc-d29919234c1f"
...
> 로컬 개발 환경과 프로젝트 구조에 대한 기초는 [manual.md](https://app.notion.com/p/ccumgol/manual.md)를, 이 과정에서 실제로 겪은 문제들은 [issue-report.md](https://app.notion.com/p/ccumgol/issue-report.md)를 함께 참고하세요.
...
| 단계 | 구성 요소 | 사용자가 하는 일 | 시스템에서 일어나는 일 | 결과 |
| --- | --- | --- | --- | --- |
| 1 | 내 컴퓨터 | 글 작성 / 디자인 수정 | 로컬 프로젝트 파일이 변경됨 | 홈페이지 원본 콘텐츠와 디자인이 준비됨 |
| 2 | GitHub | `git push` 실행 | 코드와 문서가 `ccumgol/myweb` 저장소에 보관됨 | 변경 이력이 GitHub에 저장됨 |
| 3 | Cloudflare Pages | 별도 서버 관리 없음 | GitHub 저장소를 받아 자동 빌드 후 전 세계에 배포 | 무료 호스팅으로 사이트가 공개됨 |
| 4 | 방문자 | `ccumgol.net` 접속 | Cloudflare가 배포된 사이트를 제공 | 방문자가 최신 홈페이지를 봄 |
...
1. **Hugo Extended 버전** 설치
  - 아래 박스의 내용을 복사해서 Terminal 창에 붙여넣고 실행
  - macOS 기준
```bash

brew install hugo # hugo 정적사이트생성기를 내 컴퓨터에 설치

hugo version # 설치된 hugo의 버전 확인, "extended" 라는 단어가 보여야 함
```
  - Windows 기준
    Windows에서는 **winget**을 사용하는 방법이 가장 간단합니다. Windows 10/11의 PowerShell 또는 터미널을 열고 아래 명령어를 실행합니다.

```powershell

winget install Hugo.Hugo.Extended # Hugo Extended 버전 설치

hugo version # 설치 확인, "extended" 라는 단어가 보여야 함
```

    만약 `winget` 명령어가 작동하지 않는다면, Microsoft Store에서 **앱 설치 관리자(App Installer)** 를 업데이트한 뒤 다시 시도하세요.
  - **테마·템플릿에 따라 추가 프로그램이 필요할 수 있음**
    Hugo 자체는 위 설치만으로 실행할 수 있습니다. 다만 사용하는 테마, 템플릿, 커스터마이징 방식에 따라 아래 프로그램 설치를 요구할 수 있습니다. 설치 전에는 선택한 테마의 공식 문서를 먼저 확인하세요.

| 프로그램 | 언제 필요한가 | 간단 설명 |
| --- | --- | --- |
| **Node.js** | Tailwind CSS, PostCSS, npm 패키지, 프론트엔드 빌드 도구를 사용하는 테마 | 웹사이트의 CSS·JavaScript를 빌드하거나 패키지를 설치할 때 사용하는 JavaScript 실행 환경입니다. |
| **Python** | 이미지 처리, 콘텐츠 변환, 자동화 스크립트를 함께 제공하는 템플릿 | 문서 변환, 파일 일괄 처리, 자동화 작업에 자주 쓰이는 범용 프로그래밍 언어입니다. |
| **Go** | Hugo Modules 방식으로 테마를 설치하거나 관리하는 경우 | Hugo가 만들어진 언어입니다. 일반적인 Hugo 실행에는 필요 없지만, Go 모듈 기반 테마 관리에는 필요할 수 있습니다. |
...
winget install OpenJS.NodeJS.LTS # Node.js LTS 버전 설치

winget install Python.Python.3.13 # Python 설치

winget install GoLang.Go # Go 설치
```
...
```
2. **Git** 설치
  Git은 내 컴퓨터의 파일 변경 이력을 관리하고, GitHub로 프로젝트를 올릴 때 사용하는 필수 도구입니다.

  - macOS에서는 보통 Homebrew 또는 Xcode Command Line Tools를 통해 설치합니다.
```bash

git --version # Git 설치 여부 확인
```
  - Windows에서는 PowerShell 또는 터미널에서 아래 명령어로 설치할 수 있습니다.
```powershell

winget install Git.Git # Git 설치

git --version # Git 설치 확인
```

  설치 후 터미널을 새로 열고 `hugo version`, `git --version`이 모두 정상 출력되면 Hugo와 Git 준비가 끝난 것입니다.
3. **GitHub 계정** (github.com)
  - [https://github.com](https://github.com/) 사이트에 접속하여 계정을 생성합니다.
  - 아래 Chapter 4 참고
4. **Cloudflare 계정** (dash.cloudflare.com)
  - [https://cloudflare.com](https://cloudflare.com/) 또는 [https://dash.cloudflare.com](https://dash.cloudflare.com/) 사이트에 접속하여 계정을 생성합니다.
  - 아래 Chapter 5 참고
...
이 프로젝트는 Hugo + **Blowfish(또는 다른 테마) 테마**를 기반으로 하되, 아래 파일들로 디자인을 완전히 커스터마이징했습니다. 테마 원본은 절대 수정하지 않고(서브모듈), **프로젝트 쪽에 같은 경로의 파일을 만들어 덮어쓰는 방식**을 씁니다. 이것이 Hugo의 표준 커스터마이징 방법입니다.
...
이것은 **Blowfish 테마만의 특별한 규칙이라기보다, 테마를 프로젝트에 어떤 방식으로 넣었는지에 따라 달라지는 문제**입니다.

Hugo 테마를 사용하는 방법은 크게 세 가지가 있습니다.

| 테마 사용 방식 | 특징 | 다른 컴퓨터에서 받을 때 |
| --- | --- | --- |
| 프로젝트 안에 직접 복사 | 테마 파일이 내 저장소에 그대로 포함됨 | 일반 `git clone`만 해도 됨 |
| Git 서브모듈 사용 | 테마를 별도 Git 저장소로 연결해서 관리함 | `--recursive` 또는 `git submodule update`가 필요함 |
| Hugo Modules 사용 | Go 모듈 방식으로 테마를 불러옴 | `hugo mod get`, `hugo mod tidy` 같은 별도 관리가 필요할 수 있음 |

이 프로젝트는 `themes/blowfish`를 **Git 서브모듈 방식**으로 연결했기 때문에, 저장소를 복제할 때 테마 파일이 자동으로 함께 내려오지 않습니다. Git은 기본적으로 “여기에 다른 저장소가 연결되어 있다”는 정보만 가져오고, 실제 테마 내용은 따로 받아야 합니다. 그래서 `git clone --recursive` 또는 `git submodule update --init --recursive` 명령어가 필요합니다.

다른 Hugo 테마도 **서브모듈로 설치했다면 똑같이 이 과정이 필요**합니다. 반대로 Blowfish라도 테마 파일을 직접 복사했거나 Hugo Modules 방식으로 설치했다면 처리 방법이 달라집니다. 즉, 중요한 기준은 “어떤 테마인가?”가 아니라 **“테마를 어떤 설치 방식으로 관리하고 있는가?”** 입니다.

이 프로젝트에서는 테마 원본을 직접 수정하지 않고 업데이트 이력을 깔끔하게 관리하기 위해 서브모듈 방식을 사용했습니다. 따라서 새 컴퓨터에서 프로젝트를 받을 때는 서브모듈까지 함께 받아야 정상적으로 빌드됩니다.
...
1. 저장소 목록에서 `ccumgol/myweb` 선택 → **Begin setup**
