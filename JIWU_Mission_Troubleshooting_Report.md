...
* **원인**:
  ...
  2. HTML 정적 태그에 초기 상태 제어용인 `hidden`(display: none)과 화면 노출 시 레이아웃 형태인 `flex` 또는 `grid`를 한곳에 혼용하여 경고가 발생했습니다:
     * `build-status-container` 엘리먼트에 `hidden`과 `flex` 클래스가 동시에 선언됨
     * `dashboard-form` 엘리먼트에 `hidden`과 `grid` 클래스가 동시에 선언됨
...
* HTML 마크업 단계에서는 충돌을 방지하기 위해 `flex`와 `grid` 속성을 완전히 제거하여 linter를 통과시키고, 대신 자바스크립트가 동작하여 화면이 열리는 시점에 동적으로 해당 클래스를 주입하도록 로직을 변경했습니다.
  * **HTML 수정**:
    * `hidden flex` 조합을 `hidden` 단일 클래스로 수정 (`flex` 클래스 소거)
    * `hidden grid` 조합을 `hidden` 단일 클래스로 수정 (`grid` 클래스 소거)
  * **JS 수정**:
...
## 15. Front Matter `path` 필드로 인한 Hugo 빌드 실패

### 🔍 문제 현상
* `npm run build` 실행 시 다음 오류와 함께 빌드가 실패함:
  ```bash
  ERROR deprecated: path in front matter was deprecated in Hugo v0.144.0 and subsequently removed.
  ERROR error building site: logged 1 error(s)
  ```
* **원인**: Obsidian에서 내보낸 마크다운 파일의 Front Matter에 `path:` 필드가 포함되어 있었습니다. Hugo v0.144.0 이후 이 필드는 완전히 제거되었고, 해당 필드가 존재할 경우 빌드 자체가 실패합니다.

### 🛠️ 해결 방법
* `content/` 폴더 전체에서 `path:` 필드를 일괄 제거하는 명령어를 실행했습니다:
  ```bash
  find content/ -name "*.md" ! -name "_index.md" -exec sed -i '' '/^path:/d' {} \;
  ```
* **결과**: 39개 파일에서 `path:` 필드가 제거되어 빌드가 정상적으로 통과되었습니다.
* **재발 방지**: Obsidian wiki-sync로 파일을 가져올 때마다 `path:` 필드가 다시 추가될 수 있으므로, git push 전에 `npm run build`로 빌드를 먼저 검증하는 것을 권장합니다.

---

## 16. `git push` 거절(rejected) 오류 — 원격과 로컬 커밋 불일치

### 🔍 문제 현상
* `git push` 실행 시 다음 오류가 발생하며 푸시가 거절됨:
  ```bash
  ! [rejected]        main -> main (fetch first)
  error: failed to push some refs to 'https://github.com/ccumgol/jiwumission.git'
  hint: Updates were rejected because the remote contains work that you do not have locally.
  ```
* **원인**: Admin 페이지(`/admin/`)가 GitHub REST API를 통해 원격 저장소를 직접 수정하기 때문에, 로컬 저장소보다 원격이 앞선 커밋을 가지게 됩니다. 이 상태에서 로컬 커밋을 push하면 거절됩니다.

### 🛠️ 해결 방법
```bash
# 원격 변경사항을 로컬 위에 rebase 후 push
git pull --rebase && git push
```
* `--rebase` 옵션은 원격 커밋 위에 로컬 커밋을 깔끔하게 얹어주므로 불필요한 Merge 커밋 없이 히스토리를 유지합니다.
* **menus.toml 충돌 발생 시**: Admin 페이지와 로컬 양측 모두에서 메뉴를 수정한 경우 충돌이 발생합니다. 이때는 충돌 파일을 수동으로 병합한 뒤 다음을 실행합니다:
  ```bash
  git add config/_default/menus.toml
  GIT_EDITOR="true" git rebase --continue
  git push
  ```
* **결과**: 원격과 로컬 커밋이 통합되어 정상적으로 push됩니다.
* **권장 워크플로우**: 로컬 작업 시작 전 항상 `git pull --rebase`를 먼저 실행하세요.
