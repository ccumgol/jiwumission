...
## Issue 35. 추천 GitHub 리포지터리 리스트 구조 개편 및 자동 파싱 빌드 연동

**발생 시점**: 추천 GitHub 리포지터리 리스트가 단순 텍스트 나열 형식이어서 카테고리별 분류 및 상세 설명이 필요하고 매일 신규 리포지터리가 뉴스에서 추가될 때마다 수동으로 관리하기 힘들 때.
**현상**: 비효율적인 수동 업데이트와 카테고리 시인성 저하.
**원인**: 추천 리포지터리 관리가 고정형 마크다운으로 되어 있어, daily-it-news 블로그 포스트의 신규 추천 건이 생길 때마다 수동으로 문서에 링크를 찾아서 옮겨 적어야 했음.
**해결**: [updateRepos.js](file:///Users/gihyunpark/Desktop/jiwumission/scripts/updateRepos.js) 스크립트를 작성하여 매일 블로그 뉴스에서 고유 GitHub 링크를 추출하고 한국어 설명과 이름을 매핑 및 분류하게 자동화. `pnpm run update-repos`로 등록하고 일일 백그라운드 푸시 스크립트(`auto_push.sh`)에 삽입. 카테고리는 `###` (H3) 헤더 형식으로 개편.
**결론/교훈**: 수작업으로 목록화하던 문서는 정기적인 파싱 스크립트로 파이프라인을 자동화하여 정합성과 정적 빌드 효율을 동시에 향상시킬 수 있습니다.

---

## Issue 36. 성경개론 66권 Frontmatter 표준화 및 순서 정렬(weight) / 본문 정제

**발생 시점**: `databank/bible-overview/` 폴더에 생성된 성경 66권 문서들을 테마의 정적 리스트 페이지에 연동하려고 할 때.
**현상**: Frontmatter 규격 불일치로 리스트 페이지에 비정상 노출되고, 정렬이 파일명 문자열 순서로 되어 성경 본연의 순서(창세기~요한계시록)가 섞이며, 본문 상단에 불필요한 위키 링크 및 작업 언어 문구가 포함됨.
**원인**: 개별 마크다운 파일들의 Frontmatter에 `weight` 필드가 존재하지 않았고, 본문 내에 Obsidian 특유의 번역 링크 및 콜아웃 상자가 남아 있었음.
**해결**: 일괄 스크립트(`fixBibleFrontmatter.js` 및 `cleanBibleOverview.js`)를 작성해 66개 성경개론 파일의 Frontmatter를 일괄 갱신(`title`, `categories: ["성경개론", "자료실"]`, 성경 순번 기반 `weight: 1~66` 부여)하고, 본문 내 `[!info] 작업 언어` 콜아웃과 역본 링크 목록(`- [[../nkrv/...`)을 일괄 정제하여 빌드.
**결론/교훈**: 일괄적으로 대량의 마크다운 포맷을 변환해야 할 때는 정규식을 활용한 Node.js 일회성 배치 스크립트를 만들어 처리하는 것이 수작업 실수를 방지하고 빠른 빌드 품질을 보장합니다.

---

## Issue 37. 어드민 패널 방문자 통계용 구글 GA4 & Looker Studio 연동 및 UI 탭 추가

**발생 시점**: 사이트 방문자 통계 조회를 브라우저용 어드민 관리자 화면(`static/admin/index.html`) 내에서 통합하여 보려 할 때.
**현상**: 서버가 없는 SSG 정적 사이트 특성상 자체 방문자 수 로그 적재 및 통계 대시보드 조회가 불가능함.
**원인**: 백엔드가 없는 정적 호스팅 사이트에서 방문자 로그를 수집·표시하려면 구글 애널리틱스(GA4)와 같은 외부 트래킹 툴이 필요하고, 이를 어드민 패널 내부에서 보려면 시각화 차트가 임베드되어야 함.
**해결**: 어드민 페이지에 `방문자 통계` 탭을 개설하고, 구글 태그 매니저 ID(`google_tag_manager`) 및 구글 룩커 스튜디오 공유 임베드 주소(`analytics_embed_url`)를 저장할 수 있는 입력폼을 구현. 대시보드 주소가 있으면 화면상에서 Looker Studio Iframe 차트를 띄우고, 없으면 GA4 및 Looker Studio 연동 가이드 매뉴얼을 띄우도록 처리.
**결론/교훈**: 서버가 없는 정적 웹 앱에서는 외부 시각화 도구(Google Looker Studio 등)의 공유 임베드 링크 기능을 활용하고, 이를 관리자 패널의 설정 파일 저장 방식과 융합하여 미려하고 확장성 있는 통계 패널을 구현할 수 있습니다.
